...
function talk(type, one_off = false) {
    ...
    if (type == 'hello') {
        comment.chat('hero', [
        'Asalam Au Alaikum. Mera naam hai phurteelaa .',
        'Main ek jaan Leva mission pe hoon, mujhay apnay jaani dushman Susti ko tabah karna hai.',
        'Kya aap meri madad karein gy?',
    ...
    if (type == 'door') {
        ...
        comment.chat('hero', ['Oh No! yeh darwaza toh locked hai.', 'Collect all the coins to unlock it.'])
        if (enemy) comment.chat('hero', ['Yaad rahay Baray keeron ko maar kar bhi coins dhonday ja saktay hain.'])
    ...
    if (type == 'point') {
        comment.chat('hero', [
            'Apko mila hai coin Location upgrade!',
    ...
    if (type == 'pound') {
        comment.chat('hero', [
            'Apko mila hai Ground Pound upgrade!',
            'Press the "down" key while in the air to pound the ground.',
    ...
    if (type == 'jet') {
        comment.chat('hero', [
            'Apko mila hai Jetpack upgrade! Press the up key to fly.',
    ...
    if (type == 'health') {
        comment.chat('hero', [
            'Apko mila hai Health power up! It gives you an extra life.',
    ...
    if (type == 'super pound') {
        comment.chat('hero', [
            'Apko mila hai Super Pound power up!',
    ...
    if (type == 'effect') {
        comment.chat('hero', [
            'Iss rastay main keeray hain',
            'You can jump on them to destroy them.'
        ])
        return
    }
    if (type == 'friend') {
        comment.chat('friend', ['Suno! Kya tum bhi susti ko tabah karnay aye hou?'])
        comment.chat('hero', ['Haan, Kyun?.'])
        comment.chat('friend', ['Kya main bhi tumhari iss kaam main madad karoon?.'])
        comment.chat('hero', ['Um haan ajao ek sy bhalay dou.'])
        comment.chat('friend', ['Yay! Chal meray dost\''])

        return
    }
    if (type == 'orb1') {
        comment.chat('hero', ['Yeh hum kahan aa gaye?'])
        return
    }
    if (type == 'orb2') {
        comment.chat('hero', ['Oh no! Mera solar powered jetpack andheray main kaam nahi karta'])
        return
    }
    if (type == 'orb3') {
        comment.chat('hero', ['Dost? Kahan hou tum?'])
        comment.chat('Susti', ['\'Fiqar mat karo. Meray paas mehfooz hai'])
        comment.chat('hero', ['!!!'])
        comment.chat('Susti', [
            '\' Mainay Tumhey roknay kay liye jo kar sakta tha karliya,',
            'Waqt aa gya hai kay tumhara safar yahi khatm kia jaye'
        ])
        comment.chat('hero', ['Tum kon hou?'])
        comment.chat('susti', [
            'Main hoon susti.',
            'Mera maqsad hai duniya ko tabah karna aur tum mujhay rok nahi saktay phurteelay',
            'Ab meray inteqaam ki aag main jalou',
            'GWAHAHAHAAAA!'
        ])
...
function startGame() {
    ...
    write('A Platform Game By Billybright', .04, .49)
